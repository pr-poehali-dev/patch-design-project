...
const CategoryMenu = ({ categories, onSelectCategory, className = "" }: CategoryMenuProps) => {
  const [expandedCategory, setExpandedCategory] = useState<string | null>(null);
  
  // Имитация подкатегорий (для примера)
  const subCategories = {
    "Предметы": ["Одежда", "Мебель", "Аксессуары", "Кухонные принадлежности"],
    "Профессии, искусство, хобби": ["Рукоделие", "Спорт", "Музыка", "Рисование"],
    ...
    <div className={`border rounded-md overflow-hidden shadow-sm ${className}`}>
      {menuCategories.map((category, index) => (
        <React.Fragment key={category}>
          ...
          {expandedCategory === category && subCategories[category as keyof typeof subCategories] && (
